"""TESS TargetPixel Inspector main UI logic.
It can be used in a webapp (in `main.py`) or be embedded in a Jupyter notebook or other environments.
"""
...
from bokeh.plotting import (
    curdoc,  # TODO: curdoc() usage should be removed, replaced by a doc parameter
)
...
"""
        ...
        replace_or_append_by_name(ui_layout, ui_body)
    ...
    def add_tpf_interact_fig_with_msg():
        ...
        replace_or_append_by_name(ui_layout, msg_ui)
...
def create_lc_viewer_ui():
    ...
    def add_lc_fig():
        ...
        # add the plot (replacing existing plot, if any)
        replace_or_append_by_name(ui_layout, fig)
...
async def create_app_body_ui_from_tpf(doc, tpf, magnitude_limit=None, catalogs=None):
    ...
    log.info(
        f"Plot Skyview: {tpf}, sector={tpf.meta.get('SECTOR')}, exptime={get_exptime(tpf)}, TessCut={is_tesscut(tpf)}"
    )
    ...
    if catalogs is None:
        catalogs = ["skypatrol2", "ztf", "vsx", "gaiadr3_tic"]
