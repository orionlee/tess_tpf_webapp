...
def show_app(tic, sector, magnitude_limit=None):
    ...
    async def create_app_ui(doc):
        ...
        # log resource info after main UI (the form only, or form + SkyView pixels) is shown
        # - work related to getting catalog info will not be fully reflected
        #   (due to async nature)
        log_resource_info(f"show_app(tic={tic})")
...
if __name__.startswith("bokeh_app_"):  # invoked from `bokeh serve`
    ...
    log_resource_info("main_b4_show_app")
